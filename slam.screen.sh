...
echo "Launch faster_slam in screen session [slam]"
screen -dmS slam
screen -x -S slam -p 0 -X stuff "source ~/Workspace/algor_ws/install/setup.zsh && ros2 launch faster_lio mapping_mid360.launch.py localization_mode:=true load_robot:=true prior_map_name:=PGO rviz:=false\n"
sleep 8

echo "Checking /odometry"
timeout 2 ros2 topic echo /odometry | grep posi -A3
